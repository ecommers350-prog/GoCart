...
export default function CreateStore() {
    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if (['approved', 'rejected', 'pending'].includes(data.status)) {
                ...
                switch(data.status) {
                    case "approved":
                        setMessage("Yous store has been approved, you can now add products to your store from dashboard")
                        setTimeout(()=>router.push("/store"), 5000)
                        break;
                    case "rejected":
                        setMessage("Yous store has been rejected, contact the admin for more details")
                        break;
                    case "pending":
                        setMessage("Yous store request is pending, please wait for admin to approve your store")
                        break;
    ...
    const onSubmitHandler = async (e) => {
        e.preventDefault()
        if (!user) {
           return toast('Please login to continu')
        }
        try {
            const token = await getToken()
    ...
    if (!user) {
        return (
            <div className="min-h-[80vh] mx-6 flex item-center justify-center text-slate-400">
