...
export default function AdminApprove() {
    ...
    const fetchStores = async () => {
        try {
            // Send cookies so server can authenticate the admin session
            const { data } = await axios.get('/api/admin/approve-store', { withCredentials: true })
    ...
    const handleApprove = async ({ storeId, status }) => {
        try {
            ...
            const { data } = await axios.post('/api/admin/approve-store', { storeId, status }, { withCredentials: true })
            // refresh list
